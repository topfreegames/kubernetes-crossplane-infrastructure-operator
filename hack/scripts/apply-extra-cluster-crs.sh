...
cat hack/assets/cr/extra/test-cluster-a-mp-extra.yaml |  sed  "s/<ZONE>/${KOPS_ZONE}/g" | sed  "s/<DOMAIN>/${KOPS_DOMAIN}/g" | sed "s/<BUCKET>/${KOPS_BUCKET}/g" | kubectl apply -f - --context kind-$1